...
**Docker mount syntax:** Both read-write and readonly mounts use `-v host:container` with an optional `:ro` suffix for readonly.
...
1. **Ensures Docker is running** - Verifies Docker daemon; kills orphaned NanoClaw containers from previous runs
