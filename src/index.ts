...
async function main(): Promise<void> {
  ...
  if (!TELEGRAM_ONLY) {
    ...
    // WhatsApp connects asynchronously with its own retry logic.
    // Don't await — if WhatsApp is temporarily blocked, other channels shouldn't stall.
    whatsapp.connect().catch((err) => logger.error({ err }, 'WhatsApp connect failed'));
  }
